                ...
                <div class="panel-body">
                    <div class="adv-table">
                        <table  class="display table table-bordered table-striped" id="dynamic-table">
                            ...
                            <tbody>
                                @foreach ($awardDetails as $k => $award)
                                <tr class="gradeX">
                                <td>{{ $k+1 }}</td>
                                        <td>{{$award->award_title}}</td>
                                        <td>{{$award->description}}</td>
                                        <td>{{ $award->publication_status == 1 ? "Publish" : "Unpublish" }}</td>
                                        <td>{{ $award->image_position == 1 ? "Right" : "Left" }}</td>
                                        <td>
                                            <a href="{{ route('award-edit', ['id' => $award->id]) }}"><span class="glyphicon glyphicon-pencil text-secondary">Edit</span></a>
                                            <a href="{{ route('award-delete', ['id' => $award->id]) }}" onclick="return confirm('Do you Really want to delete?')"><span class="glyphicon glyphicon-trash text-danger">Delete</span></a>
                                        </td>
                                </tr>
                                @endforeach
                            </tbody>
