                        ...
                        <table  class="display table table-bordered table-striped" id="dynamic-table">
                            ...
                            <tbody>
                                ...
                                <tr class="gradeX">
                                        <td>{{$blog->id}}</td>
                                        <td>{{$blog->title}}</td>
                                        <td><img src="{{ asset('') }}{{$blog->feature_image}}" height="150" width="150"></td>
                                        <td>{!! str_limit($blog->details_description, 100) !!}<a href="{{ route('news') }}/{{$blog->blog_slug}}" target="_blank">See full post</a></td>
                                        <td>{{ route('news') }}/{{$blog->blog_slug}}</td>
                                        <td>
                                          <a href="{{ route('blog-edit', ['id' => $blog->id]) }}"><span class="glyphicon glyphicon-pencil text-secondary">Edit</span></a>
                                          <a href="{{ route('blog-delete', ['id' => $blog->id]) }}" onclick="return confirm('Do you Really want to delete?')"><span class="glyphicon glyphicon-trash text-danger">Delete</span></a>
                                        </td>
                                </tr>
                                @endforeach
                            </tbody>
