    ...
    <section class="panel">
        ...
        <div class="panel-body">
            <div class="position-center">
                <div class="col-md-12">
                    <form role="form" action="{{ route('career-update') }}" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group">
                            ...
                            <input type="hidden" name="id" value="{{ $career->id }}" class="form-control">
                            <input type="text" name="role" value="{{ $career->role }}" class="form-control" required>

                        </div>
                        <div class="form-group">
                                                <label>Description</label>
                                                <textarea class="form-control ckeditor" name="message" placeholder="Some Description" required>{{ $career->message }}</textarea>
                            </div>
                            <div class="form-group">
                            <label>Upload Photo</label>
                            <input type="file" name="picture" class="form-control" accept="image/*">
                            <input type="hidden" name="picture_url" value="{{ $career->picture }}" class="form-control">

                        </div>
                        <div class="form-group">
                            <label>Publication Status</label>
                            <div class="single-row">
                                <input tabindex="3" type="radio" value="1" name="publication_status" {{ $career->publication_status == 1 ? "checked" : " "  }}> Publish
                                <input tabindex="3" type="radio" value="0" name="publication_status" {{ $career->publication_status == 0 ? "checked" : " "  }}> Unpublish
                            </div>
                        </div>
