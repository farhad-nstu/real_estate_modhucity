                ...
                <div class="panel-body">
                    <div class="adv-table">
                        <table  class="display table table-bordered table-striped" id="dynamic-table">
                            ...
                            <tbody>
                                @foreach ($concernDetails as $k => $concern)
                                <tr class="gradeX">
                                <td>{{ $k+1 }}</td>
                                        <td>{{$concern->concern_title}}</td>
                                        <td>{{$concern->concern_url}}</td>
                                        <td>{!! $concern->concern_description !!}</td>
                                        <td>{{ $concern->publication_status == 1 ? "Publish" : "Unpublish" }}</td>
                                        <td>
                                            <a href="{{ route('concern-edit', ['id' => $concern->id]) }}"><span class="glyphicon glyphicon-pencil text-secondary">Edit</span></a>
                                            <a href="{{ route('concern-delete', ['id' => $concern->id]) }}" onclick="return confirm('Do you Really want to delete?')"><span class="glyphicon glyphicon-trash text-danger">Delete</span></a>
                                        </td>
                                </tr>
                                @endforeach
                            </tbody>
