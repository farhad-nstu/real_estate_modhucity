        ...
        <section class="panel">
            <header class="panel-heading wht-bg">
                <h4 class="gen-case">Inbox
                        ({{ App\Contact::where(['publication_status' => 1])->get()
                     ->count() }})
                    ...
                    <table class="table table-inbox table-hover">
                       ...
                       @if($contactMessage->status == 1)
                        <tr class="unread">
                            <td class="inbox-small-cells">
                                <a href="{{ route('contact-message-view', ['id' =>
                            $contactMessage->id]) }}"><span class="label label-info
                                   pull-left">Unread</span></a>
                            </td>
                       ...
                       @else
                           <tr>
                            ...
                            $contactMessage->id]) }}"><span class="label label-success
                                   pull-left">Read</span></a>
                               </td>
                        @endif

                            <td class="view-message dont-show"><a href="{{ route('contact-message-view', ['id' =>
                            $contactMessage->id]) }}">{{ $contactMessage->name
                            }}</a></td>
                           ...
                           @if($contactMessage->status == 0)
                                       ...
                                       <a href="{{ route('contact-message-unread', ['id' =>
                            $contactMessage->id]) }}"> <span class="label label-info pull-right">Mark as
                                               Unread</span></a>
                            ...
                            <td class="view-message text-right">{{ $contactMessage->created_at->format(' g:i a,
                           l, M d-Y')
                            }}</td>
                        </tr>
                        @endforeach
