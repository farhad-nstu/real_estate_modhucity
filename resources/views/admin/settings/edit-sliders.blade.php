        ...
        <div class="panel-body">
            <div class="position-center">
                <div class="col-md-12">
                    ...
                    @foreach($sliderDetails as $imageDetail)
                    <form role="form" action="{{ route('sliders-update-image') }}" method="post"
                        enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group">
                            <label>Image</label>
                            <input type="hidden" name="id" value="{{ $imageDetail->id }}">
            ...
            <div class="position-center">
                <div class="col-md-12">
                    ...
                    @foreach($sliderDetails as $imageDetail)
                    <form role="form" action="{{ route('sliders-update') }}" method="post"
                        enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <input type="hidden" name="id" value="{{ $imageDetail->id }}">
                        <div class="form-group">
                            <label>Slider Image Url</label>
                            <input type="url" name="slider_image_link" class="form-control" placeholder="Sliders Link" value="{{ $imageDetail->slider_image_link }}" required>
                        </div>
                        <div class="form-group">
                            <label>Slider Image Url</label>
                            <input type="number" name="slider_image_serial" class="form-control" value="{{ $imageDetail->slider_image_serial }}" placeholder="Serial/Position" required>
                        </div>
                        <div class="form-group">
                            <label>Publication Status</label>
                            <div class="single-row">
                                <input tabindex="3" type="radio" value="1" {{ $imageDetail->slider_image_serial == 1 ? "checked" : "" }} name="publication_status"> Publish
                                <input tabindex="3" type="radio" value="0" {{ $imageDetail->slider_image_serial == 0 ? "checked" : "" }} name="publication_status"> Unpublish
                            </div>
                        </div>
                        <button type="submit" name="btn" class="btn btn-info">Submit</button>
                    </form>
