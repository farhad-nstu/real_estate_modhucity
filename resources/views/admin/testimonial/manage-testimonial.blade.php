                ...
                <div class="panel-body">
                    <div class="adv-table">
                        <table  class="display table table-bordered table-striped" id="dynamic-table">
                            ...
                            <tbody>
                                @foreach ($testimonialManage as $testimonial)
                                <tr class="gradeX">
                                <td>{{$testimonial->id}}</td>
                                        <td>{{$testimonial->portfolio_client_name}}</td>
                                        <td>{{$testimonial->portfolio_text}}</td>
                                        <td>{{$testimonial->publication_status}}</td>
                                        <td>
                                            <a href="{{ route('edit-testimonial', ['id' => $testimonial->id]) }}"><span class="glyphicon glyphicon-pencil text-secondary">Edit</span></a>
                                            <a href="{{ route('delete-testimonial', ['id' => $testimonial->id]) }}" onclick="return confirm('Do you Really want to delete?')"><span class="glyphicon glyphicon-trash text-danger">Delete</span></a>
                                        </td>
                                </tr>
                                @endforeach
                            </tbody>
